...
class RobotTeleoperationServer():
    def __init__(self, cfg_file='teleop.yaml'):
        ...
        arm_config = {'control_mode': 'position',
                      'use_moveit': False}
	...
	rospy.Subscriber('joy', Joy, self._callback_joystick)
        ...
        if self.use_arm:
            ...
            self.bot.arm.set_joint_positions(np.array([0, -0.14, 0.76, 1.07, 0]),
                                             plan=False)
    ...
    def set_joint(self, joint_target):
        self.is_robot_moving = True
        self.start_time = time.time()
        self.bot.arm.set_joint_positions(joint_target,
                                         plan=False, wait=False)
    ...
    def run(self):
        ...
        while True:
            ...
            joystick_key = copy.deepcopy(self.joystick)
	    print(joystick_key)
            if joystick_key is None: #no joystick input
            ...
            if key == self.cfg['KEY_POS_PAN']:
                ...
                self.reset()
            if joystick_key is not None:
		print(joystick_key.axes[1], joystick_key.axes[0])
                self.move_base(0.25*joystick_key.axes[1],3*joystick_key.axes[0])
            self.cmd = None
