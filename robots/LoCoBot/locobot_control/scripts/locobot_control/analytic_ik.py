...
class AnalyticInverseKinematics:
    def __init__(self, base_frame='/base_link', tool_frame='/gripper_link'):
        ...
        listener.waitForTransform(base_frame, "/arm_base_link", rospy.Time(), rospy.Duration(2.0))
        listener.waitForTransform("/arm_base_link", "/elbow_link", rospy.Time(), rospy.Duration(2.0))
        listener.waitForTransform("/elbow_link", "/forearm_link", rospy.Time(), rospy.Duration(2.0))
        listener.waitForTransform("/forearm_link", "/wrist_link", rospy.Time(), rospy.Duration(2.0))
        listener.waitForTransform("/wrist_link", tool_frame, rospy.Time(), rospy.Duration(2.0))
        (trans, _) = listener.lookupTransform(base_frame, '/arm_base_link', rospy.Time(0))
        self.base_offset = trans

        (trans, _) = listener.lookupTransform('/arm_base_link', '/elbow_link', rospy.Time(0))
        self.l1 = trans[2]

        (trans, _) = listener.lookupTransform('/elbow_link', '/forearm_link', rospy.Time(0))
        self.l2 = math.sqrt(trans[0] ** 2 + trans[2] ** 2)
        self.angle2_bias = math.atan2(trans[0], trans[2])

        (trans, _) = listener.lookupTransform('/forearm_link', '/wrist_link', rospy.Time(0))
        self.l3 = trans[0]
    ...
    def ik_quat(self, trans, quat, current_config=5 * [0]):
        ...
        if test < -0.49:
            pitch = pi / 2.0
            rollminusyaw = math.atan2(nquat[1] * nquat[2] - nquat[0] * nquat[3],
                                      nquat[1] * nquat[3] + nquat[0] * nquat[2])
            yaw = math.atan2(trans[1] - self.base_offset[1], trans[0] - self.base_offset[0])
            roll = rollminusyaw + yaw
        elif test > 0.49:
            pitch = -pi / 2.0
            rollplusyaw = math.atan2(nquat[1] * nquat[2] - nquat[0] * nquat[3],
                                     nquat[1] * nquat[3] + nquat[0] * nquat[2])
            yaw = math.atan2(trans[1] - self.base_offset[1], trans[0] - self.base_offset[0])
            roll = rollplusyaw - yaw
        else:
            pitch = math.asin(-2 * (nquat[1] * nquat[3] - nquat[0] * nquat[2]))
            roll = math.atan2(nquat[3] * nquat[2] + nquat[0] * nquat[1],
                              0.5 - (nquat[1] ** 2 + nquat[2] ** 2))
            yaw = math.atan2(nquat[1] * nquat[2] + nquat[0] * nquat[3],
