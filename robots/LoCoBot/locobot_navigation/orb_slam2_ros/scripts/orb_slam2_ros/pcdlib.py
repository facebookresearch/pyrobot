...
class DepthImgProcessor:
    ...
    def __init__(self, subsample_pixs=1, depth_threshold=(0, 1.5),
        ...
        assert (type(depth_threshold) is tuple
                and 0 < len(depth_threshold) < 3) \
               or (depth_threshold is None)
        ...
        img_pixs = np.mgrid[0: self.cfg_data['Camera.height']: subsample_pixs,
                   0: self.cfg_data['Camera.width']: subsample_pixs].reshape(2, -1)
...
class PointCloudProcessor:
    ...
    def __init__(self, rgb_dir, depth_dir, cfg_filename,
                 subsample_pixs=1, depth_threshold=(0, 1.5),
        ...
        self.depth_cam = DepthImgProcessor(subsample_pixs=subsample_pixs,
                                           depth_threshold=depth_threshold,
                                           cfg_filename=cfg_filename)
        self.lock = threading.Lock()
        rospy.Subscriber(camera_traj_topic,
                         Traj, self._traj_callback, queue_size=1)
    ...
    def get_current_pcd(self):
        ...
        for KFid, ext_mat in self.extrinsic_mats.iteritems():
            if KFid not in self.pcd_pool_in_cam:
                continue
            pcd_in_cam, rgb = self.pcd_pool_in_cam[KFid]
            pcd_in_world = self.depth_cam.get_pcd_iw(pcd_in_cam,
                                                     ext_mat)
        ...
        for i in range(num_KFs):
            ...
            if KFid not in self.pcd_pool_in_cam:
                ...
                if depth_im is None:
                    continue
                rgb_im = rgb_im[:, :, ::-1]
                pcd_in_cam, rgb = self.depth_cam.get_pcd_ic(depth_im=depth_im,
                                                            rgb_im=rgb_im)
    ...
    def cam_pose_to_ext_mat(self, pose):
        ...
        rot_quat = np.array([pose.orientation.x, pose.orientation.y,
...
def main():
    ...
    parser.add_argument('--cfg_filename',
                        ...
                        type=str)
    parser.add_argument('--subsample_pixs',
                        default=1,
