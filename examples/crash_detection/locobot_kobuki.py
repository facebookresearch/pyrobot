...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Process args for moving without crashing")
    parser.add_argument('-n', '--n_secs', help='Number of seconds to run the avoiding crashing controller', type=int,
                        default=60)
    parser.add_argument('-l', '--n_loops', help='Number of loops to run the avoiding crashing controller', type=int,
                        default=1000)
    parser.add_argument('-u', '--model_url', help='URL to download model from', default=MODEL_URL)
    parser.add_argument('-s', '--save_dir', help='Directory to save model', default=SAVE_DIR)
    parser.add_argument('-d', '--visualize', help='True to visualize images at each control loop, False otherwise',
                        dest='display_images', action='store_true')
