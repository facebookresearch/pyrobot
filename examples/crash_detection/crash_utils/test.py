...
class Tester(object):
    ...
    def convert_to_PIL(self, image):
        ...
        I_list = torch.cat(map(torch.unsqueeze, map(PYTORCH_TRANSFORM, [I_center, I_left, I_right, I]), [0] * 4))
        return I_list
...
def main():
    ...
    I_left = I.crop((0, 72, 320, 217))
    I_right = I.crop((320, 72, 640, 217))
    I_list = torch.cat(map(torch.unsqueeze, map(PYTORCH_TRANSFORM, [I, I_left, I_right]), [0] * 3))
