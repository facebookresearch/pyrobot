...
class Trainer(object):
    ...
    def __init__(self, csv_filename, train_f=0.8, **kwargs):
        D_train = DroneCrashDataset(csv_filename, ROOT_DIR, train_fraction=train_f, train=True)
        D_test = DroneCrashDataset(csv_filename, ROOT_DIR, train_fraction=train_f, train=False)
        self.train_dataloader = DataLoader(D_train, batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
        self.test_dataloader = DataLoader(D_test, batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
        self.create_model()
        params = []
        ...
        for epoch in range(num_epochs):
            ...
            for phase in phases:
                ...
                for batch_id, sample_batched in enumerate(dataloader):
                    ...
                    acc_stats = np.concatenate([acc_stats, [accuracy.cpu().data.numpy()]])
...
def main():
    ...
    parser.add_argument('--csv_filename', default='control_data_0_0_0#1_0_control_data_1_1_0#2_0.csv')
    ...
    M, metrics = T.train_model(num_epochs=n_epochs, save_path=save_path, save_every=save_every, log_dir=log_dir)
