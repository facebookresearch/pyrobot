...
class FrankaGripper(Gripper):
    def __init__(self, configs):
        ...
        if not wait_for(lambda: len(self._joint_positions.keys()) > 0, timeout=2.0, timeout_msg=("FrankaGripper: Failed to get gripper joint positions. Assuming no gripper attached to robot."), raise_on_error = False ):
            return 
        self._exists = True
