...
DEFAULT_VALUES = {
    ...
    "force_upper": [
        ...
        25.0,
        25.0,
    ],  # NOTE: COLLISION VALUES HAVE TO BE HIGHER THAN CONTACT THRESHOLDS (I.E. 'UPPER' THRESHOLDS HAVE TO BE HIGHER THAN 'LOWER' THRESHOLDS) FOR DETECTING COLLISIONS CORRECTLY. THE ROBOT DETECTS THE HIGHER VALUE OF THE TWO AS COLLISION.
}
...
class CollisionBehaviourInterface(object):
    ...
    def __init__(self):
        ...
        rospy.wait_for_service(
            "/franka_ros_interface/franka_control/set_force_torque_collision_behavior",
            timeout=1.0,
        )
        self._ft_collision_behaviour_handle = rospy.ServiceProxy(
            "/franka_ros_interface/franka_control/set_force_torque_collision_behavior",
            SetForceTorqueCollisionBehavior,
        )
        self._full_collision_behaviour_handle = rospy.ServiceProxy(
            "/franka_ros_interface/franka_control/set_full_collision_behavior",
            SetForceTorqueCollisionBehavior,
        )

        rospy.loginfo("Collision behaviour services found.")

    def set_ft_contact_collision_behaviour(
        self, torque_lower=None, torque_upper=None, force_lower=None, force_upper=None
    ):
        """
        ...
        try:
            response = self._ft_collision_behaviour_handle(
                lower_torque_thresholds_nominal=torque_lower,
            ...
            rospy.loginfo(
                "CollisionBehaviourInterface: Collision behaviour change request: %s. \n\tDetails: %s"
                % ("Success" if response.success else "Failed!", response.error)
            )

        except rospy.ServiceException as e:
            rospy.logwarn(
                "CollisionBehaviourInterface: Collision behaviour change service call failed: %s"
                % e
            )
            return False
    ...
    def set_force_threshold_for_contact(self, cartesian_force_values):
        ...
        """
        return self.set_ft_contact_collision_behaviour(
            force_lower=cartesian_force_values
        )
    ...
    def set_force_threshold_for_collision(self, cartesian_force_values):
        ...
        """
        return self.set_ft_contact_collision_behaviour(
            force_upper=cartesian_force_values
        )
    ...
    def set_collision_threshold(self, joint_torques=None, cartesian_forces=None):
        ...
        """
        return self.set_ft_contact_collision_behaviour(
            torque_upper=joint_torques, force_upper=cartesian_forces
        )
    ...
    def set_contact_threshold(self, joint_torques=None, cartesian_forces=None):
        ...
        """
        return self.set_ft_contact_collision_behaviour(
            torque_lower=joint_torques, force_lower=cartesian_forces
        )
