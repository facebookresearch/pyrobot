...
class ControllerParamConfigClient:
    ...
    def __init__(self, controller_name):
        ...
        self._controller_name = (
            controller_name if controller_name[0] != "/" else controller_name[1:]
        )
    ...
    def start(self, timeout=5):
        ...
        """
        service_name = "/{}/arm/controller_parameters_config".format(
            self._controller_name
        )
        try:
            self._client = dynamic_reconfigure.client.Client(
                service_name, timeout=timeout, config_callback=self._log_update
            )
        except rospy.ROSException:
            rospy.logdebug(
                "ControllerParamConfigClient: Could not find configuration server at {}".format(
                    service_name
                )
            )
    ...
    def _log_update(self, config):
        ...
        rospy.logdebug(
            "ControllerParamConfigClient: {controller_name} config set to \n\tSmoothing Param: {smoothing_param} \n\tJoint Stiffness: {K_gains} \n\tJoint Damping: {D_gains}".format(
                controller_name=self._controller_name,
                smoothing_param=config["position_joint_delta_filter"],
                K_gains=k_gains,
                D_gains=d_gains,
            )
        )
    ...
    def set_controller_gains(self, k_gains, d_gains=None):
        ...
        assert (
            len(k_gains) == 7
        ), "ControllerParamConfigClient: k_gains argument should be of length 7!"
        ...
        if d_gains:
            assert (
                len(k_gains) == 7
            ), "ControllerParamConfigClient: d_gains argument should be of length 7!"
    ...
    def get_controller_gains(self, timeout=5):
        ...
        for k_val_ in K_GAINS_KW:
            if k_val_ in config:
                ...
                rospy.logwarn(
                    "ControllerParamConfigClient: Could not find K gain {} in server".format(
                        k_val_
                    )
                )
                return False, False
        ...
        for d_val_ in D_GAINS_KW:
            if d_val_ in config:
                ...
                rospy.logwarn(
                    "ControllerParamConfigClient: Could not find D gain {} in server".format(
                        d_val_
                    )
                )
