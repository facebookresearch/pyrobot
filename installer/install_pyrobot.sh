...
helpFunction()
{
   ...
   echo -e "\t-e Decides the path to build environments. Must be a full path."
...
if [ -z "$PYTHON_VERSION" ] || [ -z "$WORKSPACE_PATH" ] || [ -z "$ENV_PATH" ]; then
...
if [ $PYTHON_VERSION == "2" ]; then
	virtualenv_name="pyro_env"
	VIRTUALENV_FOLDER=$ENV_PATH/${virtualenv_name}
	if [ ! -d "$VIRTUALENV_FOLDER" ]; then
		...
		pip install .
		deactivate
		echo "alias load_pyro2_env='source $VIRTUALENV_FOLDER/bin/activate '" >> ~/.bashrc
	fi
fi
...
if [ $PYTHON_VERSION == "3" ]; then
	...
	PYROBOT_PYTHON3_WS=$WORKSPACE_PATH/pyro3_ws
	...
	if [ ! -d "$PYROBOT_PYTHON3_WS/src" ]; then
		...
		echo "alias load_pyro3_env='source $VIRTUALENV_FOLDER/bin/activate'" >> ~/.bashrc
 		echo "alias load_pyro3_ws='source $PYROBOT_PYTHON3_WS/devel/setup.bash'" >> ~/.bashrc
	fi
	deactivate
fi
