...
helpFunction()
{
   ...
   echo -e "\t-w Decides the directory to save your workspaces. This should be a full path."
   echo -e "\t-e Decides the directory to save you environments. This also needs to be a full path."
   exit 1 # Exit script after printing help
}

while getopts "t:p:l:w:e:" opt
do
   case "$opt" in
...
if [ -z "$INSTALL_TYPE" ] || [ -z "$PYTHON_VERSION" ] || [ -z "$LOCOBOT_PLATFORM" ] || [ -z "$WORKSPACE_PATH" ] || [ -z "$ENV_PATH" ]; then
...
if [ ! -d $WORKSPACE_PATH ]; then
	echo "Workspace path does not exist: building...";
	mkdir $WORKSPACE_PATH
	echo "Created workspace directory: continuing...";
fi

if [ ! -d $ENV_PATH ]; then
	echo "Environments path does not exist: building...";
        mkdir $ENV_PATH
        echo "Created environments directory: continuing...";
fi
...
echo "Installing at $WORKSPACE_PATH."
echo "Python $PYTHON_VERSION chosen for pyRobot installation."
echo "Installing at $ENV_PATH."
...
if [ $INSTALL_TYPE == "full" ]; then
	...
	CAMERA_FOLDER=$WORKSPACE_PATH/pyro_camera_ws
...
LOCOBOT_FOLDER=$WORKSPACE_PATH/pyro_ws
...
if [ ! -d "$LOCOBOT_FOLDER/src/pyrobot" ]; then
	cd $LOCOBOT_FOLDER/src
	git clone https://github.com/HIRO-group/pyrobot.git
	cd pyrobot
	git checkout master
...
sudo chmod +x install_orb_slam2.sh
...
if [ $PYTHON_VERSION == "2" ]; then
	...
	echo "$INSTALLER_PATH is now executable.-----------------------------"
	chmod +x $INSTALLER_PATH/install_pyrobot.sh
	source $INSTALLER_PATH/install_pyrobot.sh  -p 2

	virtualenv_name="pyro2_env"
	source $ENV_PATH/${virtualenv_name}/bin/activate
	cd $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot
...
if [ $PYTHON_VERSION == "3" ]; then
	...
	echo "$INSTALLER_PATH is now executable----------"
	chmod +x $INSTALLER_PATH/install_pyrobot.sh
	source $INSTALLER_PATH/install_pyrobot.sh -p 3 -e $ENV_PATH

	virtualenv_name="pyro3_env"
	cd $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot
	source $ENV_PATH/${virtualenv_name}/bin/activate
